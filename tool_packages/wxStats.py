...
def getFrequencyData(rawdata,fre="day"):
    ...
    for it in items:
        # if it ==items[0]:
        #     nav_list=list(df_nav[it])
        #     df=freq_df_construct(datalist, nav_list, fre)
        #     # df.head()
        #     result=pd.DataFrame(np.zeros([df.shape[0],len(items)]),index=df.index,columns=items)
        #     result[it]=df["nav"]
        # else:
...
def standard_deviation(asset, start, end):  # 收益波动率函数
...
SIMPLE_STAT_FUNCS = [
    ...
    cagr,                                   #
    value_at_risk,                          #在险价值
    conditional_value_at_risk,              #条件在险价值



    # my own func
    wxTest,
    GetReturnData,                          #获取收益矩阵
    getFrequencyData,                       #获取不同频率日期
